...
def enhancedFeatureExtractorDigit(datum):
    ...
    ## DESCRIBE YOUR ENHANCED FEATURES HERE...
        number of space segments separated by hand written digit
    ##
    """
    # True if pixel pos is in bounds of image (DIGIT_DATUM_WIDTH * DIGIT_DATUM_HEIGHT)
    def inBounds(pos):
        x, y = pos
        return x in range(DIGIT_DATUM_WIDTH) and y in range(DIGIT_DATUM_HEIGHT)

    # get neighbour pixels for given pixel position
    ...
    # breadth first search, adds pixels of the same connected segment where x, y is located
    def bfs(x, y, visited):
        queue = []
        s = (x, y)
        queue.append(s)
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if features[(x, y)] == 0 and (x, y) not in visited:
                bfs(x, y, visited)
                segments += 1 # new segment visited, so we increase counter
    
    features["singleSegment"] = int(segments == 1) # 1, 2, 3, 5, 7, (maybe 4)
    features["doubleSegment"] = int(segments == 2) # 6, 9, (maybe 4)
    features["tripleSegment"] = int(segments == 3) # 8
